"""In-memory card store used by the simulation viewer.

The viewer accepts a pasted MTGA decklist, so on startup we load every
``ParsedCard`` we have on disk (across the configured sets) and build
two indices:

* ``by_id[(set_upper, collector_number)]`` — exact lookup for decklists
  that include the ``(SET) NUMBER`` suffix MTGA exports.
* ``by_name[name_lower]`` — fallback for decklists that omit the suffix
  (older MTGA exports) and for cards whose printed set in the decklist
  differs from any printing we have on file. Returns the first
  matching printing on collision.

We deliberately do **not** call ``mulligan_coach_cards.loader.load_all_cards``
the way ``card_viewer/data.py`` does — the simulation viewer doesn't
display card images, so the 170 MiB Scryfall dump is dead weight here.

## Basic lands

The persistent store at ``data/processed/parsed_cards/<SET>.json`` does
**not** contain basic lands (the parser doesn't emit them, and they're
mechanically identical across sets). Every Limited decklist references
~17 of them, so we synthesise the five basics here using the same
construction the simulation tests use
(see ``packages/simulation/tests/_factories.py:49-65``). They live in
``by_name`` only — when a decklist line says
``"7 Plains (TLA) 270"``, the ``(TLA, 270)`` lookup misses and we fall
back to the name-only index.
"""
...
def _sets_from_env() -> list[str] | None:
    ...
    ``SIMULATION_VIEWER_SETS=TLA`` (or ``TMT,ECL,TLA``) lets the user
    narrow which sets get loaded; useful when iterating on one specific
    format. Empty / unset → auto-detect every set in
    ``data/processed/parsed_cards/``.
    """
    raw = os.environ.get("SIMULATION_VIEWER_SETS", "").strip()
    if not raw:
        return None
...
def discover_sets() -> list[str]:
    ...
    Reads ``data/processed/parsed_cards/`` and returns each ``<SET>.json``
    stem, sorted for stable startup logging. Returns ``[]`` on a fresh
    checkout where ``run-detector`` hasn't been invoked yet — the caller
    can then surface a helpful "no cards loaded" state instead of crashing.
    """
...
def _basic(name: str, color: ManaOption, subtype: str) -> ParsedCard:
    ...
    return ParsedCard(
        ...
        # printings of the same basic don't matter for simulation;
        # we use distinct ids here purely for hygiene.
...
class CardStore:
    """All loaded cards plus the two indices the viewer needs."""
    ...
    def build(cls, set_codes: list[str] | None = None) -> CardStore:
        ...
        for card in entries:
            ...
            # actual basic-land printings on disk (today there are
            # none, but the load_parsed_cards return is the source of
            # truth for ids).
            ...
            for key in _name_keys(card.name):
                # ``setdefault`` so the first set we load wins on
                # name collisions across reprints — deterministic
                # but otherwise unimportant for our use case.
