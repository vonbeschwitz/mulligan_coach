...
The website surfaces both halves so the user sees one coherent
report — "40 cards, 18 unique" plus, separately, "these 2 lines
couldn't be resolved". We deliberately don't raise
:class:`DeckEncodingError` from here: ``simulate`` itself will
hard-fail if a bad deck reaches it, but the validation panel needs
to render even when the deck has problems.
...
Premier Draft decks have no sideboard; Sealed pools may. We always
stop at ``Sideboard`` / ``Commander`` / ``Companion`` so those zones
don't end up shuffled into the goldfishing library.
"""
...
class DeckCard(BaseModel):
    ...
    Stores ``count`` separately from the ``ParsedCard`` so the deck
    round-trips cleanly for multi-copy entries — we never duplicate
    ``ParsedCard`` instances at this layer; :func:`expand_deck` does
    the flattening when the simulator needs it.
    """
...
class ParseIssue(BaseModel):
    ...
    ``reason`` is a short machine-friendly tag the template can group
    by; ``message`` is the user-facing explanation.
    """
...
class ParseResult(BaseModel):
    ...
    AND ``total_cards`` lands in the Limited range (40-41). The
    website's validation panel renders both: success block when
    valid, per-line issue list otherwise. Both halves are populated
    whenever they apply — a deck with one bad line still surfaces
    the good ones in ``cards``.
    """
    ...
    def is_valid(self) -> bool:
        """Resolves cleanly AND has the right size for the simulator.

        The model requires deck=40 (Limited). 41-card decks (Yorion-
        style, also seen as "+1 land" Sealed builds) are tolerated by
        the parser but flagged at the route layer if the model needs
        exactly 40.
        """
...
# part of the goldfishing library. Match exactly (case-insensitive) so
# we don't accidentally drop a card whose name happens to contain one
# of these words.
...
def parse_mtga_decklist(text: str, store: CardStore) -> ParseResult:
    ...
    Always returns a :class:`ParseResult`; never raises for user-
    facing errors (bad lines / missing cards / incomplete encodings).
    The caller's UI then decides how to render success vs. issues.
...
def _encoding_problem(card: ParsedCard) -> str | None:
    ...
    We can't call ``check_deck_encodings`` directly because it raises
    a single combined exception over the whole deck — we want one
    issue per offending line so the user can fix them in batches. Keep
    these rules in lock-step with
