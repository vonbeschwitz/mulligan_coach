"""Tests for the set-code one-hot cache patch (active: v3 -> v4, + HOB).

Everything runs on synthetic chunk parquets built directly with pyarrow
(a "5-column pre-HOB vocab + expansion" shard, including an
...
* the six one-hots are rewritten correctly and every other column is
...
* meta-less (legacy) dirs are skipped untouched (unknown provenance);
...
* a migration whose pinned vocabulary no longer matches the live
  builder vocabulary is rejected outright.
"""
...
# (HOB rows can't exist in a real v3 cache — no 17Lands HOB data — but the
# patch is defined purely over ``expansion`` so we exercise the column
# anyway.)
_EXPANSIONS = ["TMT", "ECL", "TLA", "SOS", "MSH", "ZZZ"]

# The vocabulary a *source* (features-v3) chunk already carries.
_SOURCE_SETS = SET_ONEHOTS_V1.known_sets

_TARGET_SETS = SET_ONEHOTS_V2.known_sets


def _write_v3_chunk(path: Path, expansions: list[str]) -> pa.Table:
    """Write a synthetic v3 chunk: the pre-HOB one-hots + expansion + two
    ...
    for s in _SOURCE_SETS:
        columns[f"set_code_{s}"] = pa.array(
            [1.0 if e == s else 0.0 for e in expansions], pa.float64()
        )
    columns["expansion"] = pa.array(expansions, pa.string())
    columns["draft_id"] = pa.array([f"d-{i:03d}" for i in range(n)], pa.string())
...
# Default meta versions for a "standard v3 shard" fixture. Module-level so
# the factory's default isn't a mutable literal (B006); no test mutates it.
_V3_META_VERSIONS: dict[str, int] = {"simulation": 2, "features": 3}


def _make_shard(
    tmp_path: Path,
    *,
    versions: dict[str, int] | None = _V3_META_VERSIONS,
    ...
    original = _write_v3_chunk(shard_dir / "chunk_00000000.parquet", expansions or _EXPANSIONS)
...
# ---------------------------------------------------------------------------
# Happy path: six columns, untouched columns bit-identical
# ---------------------------------------------------------------------------


def test_patch_produces_six_columns_and_preserves_untouched(tmp_path: Path) -> None:
    ...
    # All six one-hots present and float64.
    for s in _TARGET_SETS:
    ...
    for i, exp in enumerate(_EXPANSIONS):
        row = {s: patched.column(f"set_code_{s}")[i].as_py() for s in _TARGET_SETS}
        if exp in _TARGET_SETS:
            assert row[exp] == 1.0
            assert sum(row.values()) == 1.0
        else:
            assert sum(row.values()) == 0.0

    # The new HOB column is all-zero (no HOB rows exist in a v3 cache).
    assert patched.column("set_code_HOB").to_pylist() == [0.0] * len(_EXPANSIONS)
...
# ---------------------------------------------------------------------------
# Meta bump: features 3 -> 4, patch_history, unknown keys preserved
# ---------------------------------------------------------------------------
...
def test_meta_bumped_and_unknown_keys_preserved(tmp_path: Path) -> None:
    ...
    assert meta["pipeline_versions"] == {"simulation": 2, "features": 4}
    ...
    entry = history[0]
    assert entry["patch"] == "set_onehots_v2"
    assert entry["from_features"] == 3
    assert entry["to_features"] == 4
    assert "at" in entry


def test_meta_bump_appends_to_existing_patch_history(tmp_path: Path) -> None:
    """A shard already patched once (v1 -> v2 era) keeps its old history
    entry when the v2 migration appends its own."""
    prior = [{"patch": "set_onehots_v1", "at": now_iso(), "from_features": 1, "to_features": 2}]
    shard_dir, _ = _make_shard(tmp_path, extra_meta={"patch_history": prior})

    patch_shard_dir(shard_dir, dry_run=False)

    history = json.loads((shard_dir / SHARD_META_FILENAME).read_text())["patch_history"]
    assert [e["patch"] for e in history] == ["set_onehots_v1", "set_onehots_v2"]
...
def test_second_run_skips_and_chunk_bytes_identical(tmp_path: Path) -> None:
    ...
    result = patch_shard_dir(shard_dir, dry_run=False)
    assert result.disposition is Disposition.SKIPPED_AT_TARGET
...
# ---------------------------------------------------------------------------
# Legacy dir (no meta): skipped untouched — unknown provenance
# ---------------------------------------------------------------------------


def test_legacy_dir_skipped_untouched(tmp_path: Path) -> None:
    shard_dir, _ = _make_shard(tmp_path, versions=None)  # no _meta.json
    chunk = shard_dir / "chunk_00000000.parquet"
    bytes_before = chunk.read_bytes()

    result = patch_shard_dir(shard_dir, dry_run=False)
    assert result.disposition is Disposition.SKIPPED_LEGACY
    assert result.meta_bumped is False
    assert not (shard_dir / SHARD_META_FILENAME).exists()
    # Chunk untouched: no HOB column, bytes identical.
    assert chunk.read_bytes() == bytes_before
    assert "set_code_HOB" not in _read(shard_dir).schema.names


# ---------------------------------------------------------------------------
# Refused: unrecognised version combination (e.g. a pre-Step-5 v1 shard)
# ---------------------------------------------------------------------------


def test_unexpected_versions_refused_and_chunk_untouched(tmp_path: Path) -> None:
    shard_dir, _ = _make_shard(tmp_path, versions={"simulation": 1, "features": 1})
    chunk = shard_dir / "chunk_00000000.parquet"
    bytes_before = chunk.read_bytes()
    ...
    # Not patched: no HOB column, bytes untouched.
    assert chunk.read_bytes() == bytes_before
    assert "set_code_HOB" not in _read(shard_dir).schema.names
...
def test_dry_run_writes_nothing(tmp_path: Path) -> None:
    ...
    # The dry run already announces the column the patch would add.
    assert result.chunk_reports[0].set_columns_added == ["set_code_HOB"]
...
# ---------------------------------------------------------------------------
# Crash recovery: chunks patched but meta left at 3 -> re-run finishes
# ---------------------------------------------------------------------------
...
def test_crash_between_chunk_and_meta_recovers_idempotently(tmp_path: Path) -> None:
    ...
    assert first.set_columns_added == ["set_code_HOB"]
    # Meta is still at features 3 (never bumped).
    meta = json.loads((shard_dir / SHARD_META_FILENAME).read_text())
    assert meta["pipeline_versions"]["features"] == 3

    # Patching the same chunk again is idempotent (same values).
    values_once = _read(shard_dir).column("set_code_HOB").to_pylist()
    patch_chunk(chunk)
    assert _read(shard_dir).column("set_code_HOB").to_pylist() == values_once

    # The full dir-level re-run still sees v3 meta, re-patches, and bumps.
    result = patch_shard_dir(shard_dir, dry_run=False)
    assert result.disposition is Disposition.PATCHED
    assert result.meta_bumped is True
    meta2 = json.loads((shard_dir / SHARD_META_FILENAME).read_text())
    assert meta2["pipeline_versions"]["features"] == 4


# ---------------------------------------------------------------------------
# patch_chunk guards + patch_roots scan + stale-migration rejection
# ---------------------------------------------------------------------------
...
def test_patch_roots_scans_nested_dirs_and_reports(tmp_path: Path) -> None:
    ...
    for set_code in ("TLA", "SOS"):
        ...
        _write_v3_chunk(d / "chunk_00000000.parquet", _EXPANSIONS)
        _write_meta(d, {"simulation": 2, "features": 3})
    ...
    assert "set_onehots_v2" in text


def test_patch_roots_rejects_stale_migration(tmp_path: Path) -> None:
    """Reusing a retired migration (pinned vocabulary != live builder
    vocabulary) fails loudly before touching anything."""
    with pytest.raises(CachePatchError, match="set_onehots_v1"):
        patch_roots([tmp_path], dry_run=True, migration=SET_ONEHOTS_V1)


def test_active_migration_matches_live_vocabulary() -> None:
    """If this fails, DEFAULT_KNOWN_SETS changed without adding a new
    pinned migration — add one (and bump FEATURES_SEMANTICS_VERSION)."""
    from mulligan_coach_features import DEFAULT_KNOWN_SETS

    assert tuple(DEFAULT_KNOWN_SETS) == ACTIVE_MIGRATION.known_sets
