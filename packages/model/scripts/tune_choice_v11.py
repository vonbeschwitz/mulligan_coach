"""Train choice_v11: the v10 sweep re-run with MSH added to training.

This is a copy of ``tune_choice_v10.py`` — same 6-config hyperparameter
sweep, same event types (PremierDraft + TradDraft), same grouped
``draft_id`` split at ``seed=0``, same selection metric (lowest
PremierDraft validation log-loss). Two differences:

* **MSH joins the training sets** (TLA + TMT + SOS + MSH). The MSH
  choice caches were materialised 2026-08-09 at {simulation: 2,
  features: 3} — the same sim semantics as the v10 caches. The
  out-of-sample cohort eval that had to happen *before* MSH entered
  training (scripts/elite_first_mull_results.md, logs/msh_oos/) was run
  on 2026-08-09; once MSH is trained on, that experiment is
  unrepeatable.
* **FEATURES_SEMANTICS_VERSION 4** — the HOB set-vocabulary bump. All
  eight input caches were upgraded in place by the ``set_onehots_v2``
  cache patch (adds an all-zero ``set_code_HOB`` column; no simulated
  value changes), so v11's feature list includes ``set_code_HOB`` and a
  promoted v11 clears both the pipeline-version and the
  set-unknown-to-model degradations for HOB decks.

Like v10, this model is the candidate for promotion to
``models/choice_prod``.

Run (after patch_set_onehots.py has upgraded the caches to features 4):
    .venv/Scripts/python.exe packages/model/scripts/tune_choice_v11.py
"""
...
LOG_PATH = REPO_ROOT / "logs" / "tune_choice_v11.log"
OUTPUT_DIR = REPO_ROOT / "models" / "choice_v11"

# v10's sets + MSH (its choice caches landed 2026-08-09).
SETS = ("TLA", "TMT", "SOS", "MSH")
...
# Same focused config set choice_v6 / v7 / v9 / v10 swept over.
CONFIGS = [
...
def setup_logger() -> logging.Logger:
    ...
    log = logging.getLogger("tune_choice_v11")
...
def main() -> None:
    log = setup_logger()
    log.info("==== choice_v11 sweep (choice_v10 methodology + MSH + features-v4 vocab) ====")
    ...
    # Per-set test log-loss — with MSH new to training, show its in-domain
    # fit alongside the incumbents (audit aid; selection stays Premier-val).
    exp_test = df["expansion"].to_numpy()[splits.test]
    log.info("\nPer-set test log_loss:")
    for s in SETS:
        mask = exp_test == s
        if mask.any():
            m = _compute_metrics(y_test[mask], p_test[mask])
            log.info("  %-4s n=%d  log_loss=%.4f  acc=%.4f", s, m.n_rows, m.log_loss, m.accuracy)
