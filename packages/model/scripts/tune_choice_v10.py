"""Train choice_v10: the v9 sweep re-run on SIMULATION-v2 caches.

This is a verbatim copy of ``tune_choice_v9.py`` — same 6-config
...
caches (plus the TLA/TMT/ECL win caches that feed kept-hand reuse)
were re-materialised starting 2026-07-09, picking up:

* **SIMULATION_SEMANTICS_VERSION 2** — the mana-solver
  one-activation-per-source fix (PR #112). Under v1 a land with two
  mana abilities (Gleaming Bastion, Surveillance Room) could
  "double-tap" for two mana in one payment, inflating castability
  stats — e.g. 2-drops reported ~3% turn-1 castable whenever the
  deck's single Bastion could be drawn.
* **Card-encoding fixes merged since the 2026-07-04 remat** — any
  owner-ruling encoding changes to TLA/TMT/SOS cards (e.g. the
  ETB-credit ruling of PR #107) are absorbed by the fresh caches.

Like v9, this model is the candidate for promotion to
``models/choice_prod`` — after which model and simulator semantics
match again and the pipeline-version degradation clears.

Run (after the 2026-07-09 re-materialisation):
    .venv/Scripts/python.exe packages/model/scripts/tune_choice_v10.py
"""
...
LOG_PATH = REPO_ROOT / "logs" / "tune_choice_v10.log"
OUTPUT_DIR = REPO_ROOT / "models" / "choice_v10"
...
def setup_logger() -> logging.Logger:
    ...
    log = logging.getLogger("tune_choice_v10")
...
def main() -> None:
    log = setup_logger()
    log.info("==== choice_v10 sweep (choice_v9 methodology + simulation-v2 caches) ====")
