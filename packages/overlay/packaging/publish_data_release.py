...
4. Ensures the ``data-current`` GitHub Release exists (creates it
   if missing), then uploads each artifact + the manifest itself
   with ``gh release upload --clobber`` so re-runs replace assets
   in place rather than tearing down the release.
